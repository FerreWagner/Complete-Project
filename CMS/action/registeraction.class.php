	...
	class RegisterAction extends Action {
		...
		private function reg() {
			if (isset($_POST['send'])) {
				...
				if (Validate::checkLength($_POST['user'],2,'min')) Tool::alertBack(' 用户名长度不得小于两位！');
				if (Validate::checkLength($_POST['user'],20,'max')) Tool::alertBack(' 用户名长度不得大于二十位！');
				...
				$this->_model->state = 1;
				$this->_model->time = time();   //这里做注册状态的时间记录
				if ($this->_model->checkUser()) Tool::alertBack(' 用户名重复！');
				if ($this->_model->checkEmail()) Tool::alertBack(' 邮件重复！');
		...
		public function login() {
			if (isset($_POST['send'])) {
				...
				if (!!$_user = $this->_model->checkLogin()) {
					...
					$_cookie->setCookie();
					$this->_model->id = $_user->id;    //通过$_user用checkLogin()方法得到的id拿过来，下面拿到setLaterUser()里做id对比
					$this->_model->time = time();      //这里做登录状态的事件记录
