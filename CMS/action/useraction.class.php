<?php
	class UserAction extends Action {
		...
		private function question($_question) {
			$_questionArr = array('您最喜欢做的事情？','您的宗教看法？','您最欣赏的人？');
		...
		private function face($_face) {
			...
			foreach ($_one as $_value) {
				if ('0'.$_value.'.jpg' == $_face) $_selected='selected="selected"';
				@$_html .= '<option '.$_selected.' value="tou'.$_value.'.jpg">0'.$_value.'.jpg</option>';
				$_selected = '';
			}
			foreach ($_two as $_value) {
				if ($_value.'.jpg' == $_face) $_selected='selected="selected"';
				$_html .= '<option '.$_selected.' value="tou'.$_value.'.jpg">'.$_value.'.jpg</option>';
				$_selected = '';
			}
