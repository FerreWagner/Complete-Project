...
<body id="main">
<div class="map">
   内容管理 &gt;&gt;查看文档列表    &gt;&gt;<b id="title">{$title}</b>
</div>
    ...
    {if $show}
        <table cellspacing=0>
            ...
            <tr><td><script type="text/javascript" >document.write({@key+1}+{$num});</script></td><td><a href="../details.php?id={@value->id}" title="{@value->t}" target="_blank">{@value->title}</a></td><td>{@value->attr}</td><td><a href="?action=show&nav={@value->nav}">{@value->nav_name}</a></td><td>{@value->count}</td><td>{@value->date}</td><td><a href="content.php?action=update&id={@value->id}">修改 </a> | <a href="content.php?action=delete&id={@value->id}" onclick="return confirm('确定删除?') ? true : false"> 删除</a></td></tr>
                {/foreach}
                {else}
    ...
    <table cellspacing=0 class="content">
            ...
            <tr><td>&nbsp栏 目 ： <select name="nav"><option value="" style="padding:0">请选择一个栏目类别</option>{$nav}</select><span class="blue"> 【必选】</span></td></tr>
            ...
            <tr><td>&nbsp作 者 ： <input type="text" name="author" value="{$author}" class="text" /> (* 作者名10位内)</td></tr>
    ...
    <table cellspacing=0 class="content">
            <tr><th><b>发布一条新文档</b></th></tr>
            <tr><td>文档标题：<input type="text" name="title" value="{$titlec}" class="text" /><span class="blue"> 【必填】</span> (* 标题2-50字符)</td></tr>
            <tr><td>&nbsp栏 目 ： <select name="nav"><option value="" style="padding:0">请选择一个栏目类别</option>{$nav}</select><span class="blue"> 【必选】</span></td></tr>
            <tr><td>定义属性：{$attr}
            </td></tr>
            <tr><td>TAG标签： <input type="text" name="tag" value="{$tag}" class="text" /> (* 每个标签用','隔开,总长度30)</td></tr>
            <tr><td>关键字 ： <input type="text" name="keyword" value="{$keyword}" class="text" /> (* 每个关键字用','隔开,总长度30)</td></tr>
            <tr><td>缩略图 ： <input type="text" name="thumbnail" class="text" value="{$thumbnail}" readonly="readonly" />
                          <input type="button" value="上传缩略图" onclick="centerWindow('../templates/upfile.html','upfile','600','400')" />
                          <img name="pic" src="{$thumbnail}" style="display:block;" /> (* 缩略图2M内,格式:JPG GIF PNG)
            </td></tr>
            <tr><td>文章来源：<input type="text" name="source" value="{$source}" class="text" /> (* 文章来源20位之内)</td></tr>
            <tr><td>&nbsp作 者 ： <input type="text" name="author" value="{$author}" class="text" /> (* 作者名10位内)</td></tr>
            <tr><td><span class="middle">内容摘要：</span><textarea name="info">{$info}</textarea> <span class="middle">(* 内容摘要200内)</span></td></tr>
            <tr class="ckeditor"><td><textarea id="TextArea1" name="content" class="ckeditor">{$content}</textarea></td></tr>
            <tr><td>评论选项：{$commend}
            &nbsp&nbsp&nbsp&nbsp 浏览次数：<input type="text" name="count" value="{$count}" class="text small" />
            <tr><td>文档排序：<select name="sort">
                                {$sort}
                           </select>
            &nbsp&nbsp&nbsp&nbsp 消费金币：<input type="text" name="gold" value="{$gold}" class="text small" />
            <tr><td>阅读权限：<select name="readlimit">
                                {$readlimit}
                           </select>
            &nbsp&nbsp&nbsp&nbsp标题颜色： <select name="color">
