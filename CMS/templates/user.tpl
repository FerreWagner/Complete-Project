...
<form method="post" name="reg">
	<table cellspacing="0" class="user">
		...
		<tr><td>选择头像：<select name="face" onchange="sfacee();">
									...
									<option value="tou{@value}.jpg">{@value}.jpg</option>
									{/foreach}
									{foreach $OptionFaceTwo(key,value)}
									<option value="zhan{@value}.jpg">{@value}.jpg</option>
									{/foreach}					
								</select>
		</td></tr>
...
<form method="post" name="reg">
	...
	<table cellspacing="0" class="user">
		...
		<tr><td>电子邮件：<input type="text" class="text" value="{$email}" name="email" /> <span class="red">[必填]</span> ( *每个电子邮件只能注册一个ID )</td></tr>
		<tr><td>选择头像：<select name="face" onchange="sfacee();">
									{$face}			
								</select>
		</td></tr>
